...
- **User rules** — `~/.countersignal/cxp/rules/*.yaml`, loaded at runtime, override built-in rules with the same ID
...
SQLite database at `~/.countersignal/cxp.db`. Schema version tracked via `PRAGMA user_version`.
